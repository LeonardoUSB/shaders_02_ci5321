...
export class Giro_Animado implements RasenganPhase {
    ...
    constructor() {
        ...
        // A. Ejes X (rojo), Y (verde), Z (azul) para orientarnos
        this.axisHelper = new THREE.AxesHelper(3); // Tamaño 3
        this.container.add(this.axisHelper);

        // B. Esfera de Alambre (Wireframe) de radio 2.0
        // Usamos pocas divisiones (16x12) para que sea clara y no sature visualmente
        const sphereGeo = new THREE.SphereGeometry(2.0, 16, 12); 
        ...
        // 1. GEOMETRÍA: Una sola partícula

        const axesCount = 10;
        const particlesPerAxis = 8000; // Ajusta para más o menos densidad
        ...
        for (let i = 0; i < axesCount; i++) {
            for (let j = 0; j < particlesPerAxis; j++) {
                ...
                ids[index] = i; // El ID del eje (0-9)
                // Repartimos las partículas uniformemente (0.0 a 1.0)
